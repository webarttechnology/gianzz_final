...
class HomeController extends Controller
{
    ...
    public function dashboard(Request $request){
        if($request->method() == 'GET'){     
            ...
            if($duplicateEmailCheck == 0){
                ...
                if($registration){
                    return Redirect::to('/edit-profile')-> with('successmsg',"Profile Updated Succesfully")->withInput($request->all);
                }else{
                    return Redirect::to('/edit-profile')-> with('errmsg',Config::get('constants.UPDATE_ERROR'))->withInput($request->all);
                }
            }else{
                return Redirect::to('/edit-profile')-> with('errmsg',"Email Id Already Register")->withInput($request->all);
            }
        }
    }
    ...
    public function passwordchange(Request $request){
        if($request->method() == 'GET'){  
            ...
            if($password_update){
                return Redirect::to('/password-change')-> with('successmassage',"Password Update Succesfully")->withInput($request->all);
            }else{
                return Redirect::to('/password-change')-> with('errormsg',"Password Update Not Succesfull")->withInput($request->all);
            }
